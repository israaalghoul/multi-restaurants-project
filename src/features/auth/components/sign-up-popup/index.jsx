...
export function SignUpPopup() {
  const { modal, openLogin, closeModal }  = usePopupStore();
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && closeModal()}>
      <DialogContent className=" w-[450px] bg-background p-6 rounded-lg border-0 flex flex-col items-center">
        <DialogHeader>
        ...
        <div>
          ...
          onClick={()=> navigate(appRoutes.reverseTable)}>
            Sign up
          </Button>
          ...
          <p className="text-center text-sm text-foreground mt-3">
            ...
            <span className="text-primary ml-2 font-medium cursor-pointer"
            onClick={openLogin}
            >
              Log in
            </span>
